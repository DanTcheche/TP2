...
/*
	hash_t* result = hash_crear(NULL);
	hash_iter_t* iter = hash_iter_crear(hash);

	while(!hash_iter_al_final(iter)){

		char* user = hash_iter_ver_actual(iter);
		int* key = hash_obtener(hash,user);		
		char* user_aux = hash_obtener(result,(char*)key);
		...
		if(user_aux){
			...
			strv[2] = NULL;
			char* user = join(strv,',');
			hash_guardar(result,(char*)key,user);
	...
	free(iter);
	free(hash);
	hash_iter_t* iter_2 = hash_iter_crear(result);

	while(!hash_iter_al_final(iter_2)){
	
		char* amount = hash_iter_ver_actual(iter_2);
		char* users = hash_obtener(result,amount);
		printf("%d: %s\n", (int)*amount, users);
		hash_iter_avanzar(iter_2);
	}
	free(iter_2);
	free(result);
